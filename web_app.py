...
try:
    ...
    try:
        from db_web import DatabaseManager
        from gst_calculator_simple import GSTCalculator
    except ImportError:
        # Final fallback - embed database directly in web app
        import sqlite3
        import json
        ...
        class DatabaseManager:
            ...
            def init_database(self):
                try:
                    ...
                    cursor.execute('''CREATE TABLE IF NOT EXISTS shop_settings (
                        id INTEGER PRIMARY KEY, key TEXT UNIQUE, value TEXT, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)''')
                    cursor.execute('''CREATE TABLE IF NOT EXISTS customers (
                        id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL, phone TEXT, email TEXT, 
                        address TEXT, gstin TEXT, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, 
                        updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)''')
                    cursor.execute('''CREATE TABLE IF NOT EXISTS products (
                        id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL, description TEXT, 
                        price REAL NOT NULL, gst_percent REAL DEFAULT 18, stock_quantity INTEGER DEFAULT 0, 
                        category TEXT, barcode TEXT, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, 
                        updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)''')
                    cursor.execute('''CREATE TABLE IF NOT EXISTS invoices (
                        id INTEGER PRIMARY KEY AUTOINCREMENT, invoice_number TEXT UNIQUE NOT NULL, 
                        customer_id INTEGER, items TEXT NOT NULL, subtotal REAL NOT NULL, gst_amount REAL NOT NULL, 
                        sgst_amount REAL NOT NULL, cgst_amount REAL NOT NULL, total_amount REAL NOT NULL, 
                        payment_method TEXT, notes TEXT, status TEXT DEFAULT 'pending', 
            ...
            def add_default_settings(self):
                default_settings = {
                    'shop_name': 'Your Shop Name', 'address': 'Your Address', 'phone': 'Your Phone',
                    'email': 'your@email.com', 'gstin': 'Your GSTIN', 'upi_id': 'yourupi@upi',
                    'invoice_prefix': 'INV', 'default_gst': '18', 'default_template': 'template1'
                }
            ...
            def save_shop_setting(self, key, value):
                try:
                    conn = sqlite3.connect(self.db_path)
                    cursor = conn.cursor()
                    cursor.execute('''INSERT OR REPLACE INTO shop_settings (key, value, updated_at)
            ...
            def add_customer(self, customer_data):
                try:
                    conn = sqlite3.connect(self.db_path)
                    cursor = conn.cursor()
                    cursor.execute('''INSERT INTO customers (name, phone, email, address, gstin)
                        VALUES (?, ?, ?, ?, ?)''', (
                ...
                try:
                    ...
                    if search_term:
                        cursor.execute('''SELECT * FROM customers 
                            WHERE name LIKE ? OR phone LIKE ? OR email LIKE ? ORDER BY name''',
                    ...
                    for row in cursor.fetchall():
                        customers.append({'id': row[0], 'name': row[1], 'phone': row[2], 'email': row[3], 
                                       'address': row[4], 'gstin': row[5], 'created_at': row[6], 'updated_at': row[7]})
                    conn.close()
                    return customers
                except Exception as e:
            ...
            def get_customer(self, customer_id):
                try:
                    ...
                    row = cursor.fetchone()
                    if row:
                        customer = {'id': row[0], 'name': row[1], 'phone': row[2], 'email': row[3], 
                                   'address': row[4], 'gstin': row[5], 'created_at': row[6], 'updated_at': row[7]}
                        conn.close()
                        return customer
                    conn.close()
            ...
            def add_product(self, product_data):
                try:
                    conn = sqlite3.connect(self.db_path)
                    cursor = conn.cursor()
                    cursor.execute('''INSERT INTO products (name, description, price, gst_percent, stock_quantity, category, barcode)
                        VALUES (?, ?, ?, ?, ?, ?, ?)''', (
                        product_data['name'], product_data.get('description', ''), product_data['price'],
                        product_data.get('gst_percent', 18), product_data.get('stock_quantity', 0),
                        product_data.get('category', ''), product_data.get('barcode', '')))
                ...
                try:
                    ...
                    if search_term:
                        cursor.execute('''SELECT * FROM products 
                            WHERE name LIKE ? OR category LIKE ? OR barcode LIKE ? ORDER BY name''',
                            (f'%{search_term}%', f'%{search_term}%', f'%{search_term}%'))
                    else:
                    ...
                    products = []
                    for row in cursor.fetchall():
                        products.append({'id': row[0], 'name': row[1], 'description': row[2], 'price': row[3],
                                       'gst_percent': row[4], 'stock_quantity': row[5], 'category': row[6],
                                       'barcode': row[7], 'created_at': row[8], 'updated_at': row[9]})
                    conn.close()
                    return products
                except Exception as e:
            ...
            def create_invoice(self, invoice_data):
                try:
                    conn = sqlite3.connect(self.db_path)
                    cursor = conn.cursor()
                    cursor.execute('''INSERT INTO invoices (invoice_number, customer_id, items, subtotal, gst_amount, 
                        ...
                        json.dumps(invoice_data['items']), invoice_data['subtotal'], invoice_data['gst_amount'],
                        invoice_data['sgst_amount'], invoice_data['cgst_amount'], invoice_data['total_amount'],
                        invoice_data.get('payment_method', ''), invoice_data.get('notes', ''), 
                        invoice_data.get('status', 'pending'), invoice_data.get('payment_status', 'pending')))
            ...
            def get_invoices(self, limit=None, customer_id=None):
                try:
                    ...
                    invoices = []
                    for row in cursor.fetchall():
                        invoices.append({'id': row[0], 'invoice_number': row[1], 'customer_id': row[2],
                                       'items': json.loads(row[3]) if row[3] else [], 'subtotal': row[4],
                                       'gst_amount': row[5], 'sgst_amount': row[6], 'cgst_amount': row[7],
                                       'total_amount': row[8], 'payment_method': row[9], 'notes': row[10],
                                       'status': row[11], 'payment_status': row[12], 'created_at': row[13],
            ...
            def generate_invoice_number(self):
                try:
                    ...
                    cursor.execute('''SELECT invoice_number FROM invoices 
                        WHERE invoice_number LIKE ? ORDER BY created_at DESC LIMIT 1''', (f'{prefix}%',))
            ...
            def get_sales_summary(self, start_date, end_date):
                try:
                    conn = sqlite3.connect(self.db_path)
                    cursor = conn.cursor()
                    cursor.execute('''SELECT COUNT(*) as total_invoices, COALESCE(SUM(total_amount), 0) as total_sales,
                        COALESCE(SUM(subtotal), 0) as total_subtotal, COALESCE(SUM(gst_amount), 0) as total_gst
                        FROM invoices WHERE DATE(created_at) BETWEEN ? AND ? AND status = 'completed''', 
                        (start_date, end_date))
                    result = cursor.fetchone()
                    summary = {'total_invoices': result[0] or 0, 'total_sales': result[1] or 0,
                              'total_subtotal': result[2] or 0, 'total_gst': result[3] or 0}
                    conn.close()
                    return summary
                except Exception as e:
                    print(f"Error getting sales summary: {e}")
                    return {'total_invoices': 0, 'total_sales': 0, 'total_subtotal': 0, 'total_gst': 0}
